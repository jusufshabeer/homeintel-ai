...
import { voiceAgentConfig, hasVoiceAgent } from "@/lib/voiceAgentConfig";
...
export function RouteCta({ result }: RouteCtaProps) {
  ...
  const handleKlempnerBestellen = () => {
    if (voiceAgentConfig.shareableUrl) {
      window.open(voiceAgentConfig.shareableUrl, "_blank", "noopener,noreferrer");
      return;
    }
    if (voiceAgentConfig.agentId) {
  ...
  if (result.type === "Gig-Worker") {
    return (
      <div className="space-y-4">
        {/* Call to Action: Sprachagent für Termin (ElevenLabs + n8n + Google Kalender) */}
        {hasVoiceAgent ? (
          <>
            <Button
              variant="default"
              ...
              onClick={handleKlempnerBestellen}
              aria-label="Klempner per Sprachassistent bestellen — Termin vereinbaren"
            >
              <Wrench className="size-5" />
              Klempner bestellen
            </Button>
            <p className="text-muted-foreground text-sm">
              Per Sprachassistent einen Termin vereinbaren — verbunden mit Ihrem Kalender.
            </p>
            {voiceAgentConfig.agentId && (
              <VoiceAgentModal
                agentId={voiceAgentConfig.agentId}
                open={voiceAgentOpen}
